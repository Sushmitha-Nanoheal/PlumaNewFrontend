...
class Animator {
  ...
  addKeyFrame(timestamp, added, deleted) {
    // get time diff
    let numSections = Math.ceil(
      (timestamp - this.lastTimestamp) / this.frameLength
    );
    // Divide points into smaller keyframes
    let pointsPerSection = Math.ceil(
      (added.length + deleted.length) / numSections
    );
...
export const View = () => {
  ...
  function initialize(data) {
    ...
    window.animator.context.putImageData(window.animator.imageData, 0, 0);
  ...
  useEffect(() => {
    ...
    console.log(
      ">>>>",
      `${process.env.REACT_APP_API_URL_WEBSOCKET_PRESENTATION}${uuid}/view`
    );
    ...
    // window.viewSocket = new WebSocket(
    //   `wss://app.pluma.in/api/presentation/${uuid}/view`);
    // window.viewSocket.addEventListener('message', processMessage);
    // window.viewSocket.addEventListener('close', handleSocketClose);
  }, []);
  ...
  ///
  //   (function(){
  //     console.log(document.getElementsByClassName("App").innerHTML + "hyy")
  ...
  return (
    ...
    >
      <h2 style={{ height: "3rem" }} className="text-center">
        {presentationName}
